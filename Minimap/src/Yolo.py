...
class Yolo:
    ...
    def get_attributes(self, name):
        ...
        self.objects = results.pandas().xyxy[0]
        print(self.objects)  #print detected object
        
        self.objects = self.model(self.frame).pandas().xyxy[0]
        
            
        for i in range(len(self.objects)):
            if self.objects["name"][i] == "person" and self.objects["confidence"][i] >0.6 :
                ...
                confidence = round(self.objects["confidence"][i], 2)
    ...
    def get_coordinates(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            point.append(x)
            point.append(y)
            self.coordinates_src.append(point)